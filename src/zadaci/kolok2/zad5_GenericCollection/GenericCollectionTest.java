package kolok2.zad5_GenericCollection;
...
class GenericCollection <T extends Comparable<T> & IHasTimestamp>{
    ...
    Map<String, Set<T>> elementsBycategoryMap;

    public GenericCollection() {
        this.elementsBycategoryMap = new TreeMap<String,Set<T>>(Comparator.naturalOrder());;
    }

    public void addGenericItem(String category, T element) {
        elementsBycategoryMap.putIfAbsent(category,new TreeSet<>());
        elementsBycategoryMap.computeIfPresent(category,(k,v) -> {
            v.add(element);
            return v;
        });
    }
    ...
    public Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        return elementsBycategoryMap.values().stream()
                .flatMap(Collection::stream)
                .filter(element -> element.getTimestamp().isAfter(from) && element.getTimestamp().isBefore(to))
    ...
    public Collection<T> itemsFromCategories (List<String> categories){
       return elementsBycategoryMap.entrySet().stream()
