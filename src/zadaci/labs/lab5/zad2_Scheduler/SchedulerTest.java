package labs.lab5.zad2_Scheduler;
...
class Timestamp<T extends Comparable<T>> implements Comparable<Timestamp<T>> {
    final T element;
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(time, timestamp1.time);
    }
...
class Scheduler<T extends Comparable<T>> {
    List<Timestamp<T>> timeCollection;
    Timestamp<T> currTime;
    ...
    public boolean remove(Timestamp<T> t){
        if(timeCollection.contains(t)){
            timeCollection.remove(t);
            return true;
        } else
            return false;
    }
    ...
    public Timestamp<T> next(){
        return timeCollection.stream().sorted().filter(el -> el.time.isAfter(LocalDateTime.now())).findFirst().orElse(null);
    }

    public Timestamp<T> last(){
        return timeCollection.stream().sorted(Comparator.reverseOrder()).filter(el -> el.time.isBefore(LocalDateTime.now())).findFirst().orElse(null);
    }

    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return timeCollection.stream()
                .filter(el -> el.time.isAfter(begin) &&  el.time.isBefore(end))
...
//            System.out.println(scheduler.currTime);
//            //TODO
//            System.out.println(scheduler.timeCollection);
    ...
    private static <T extends Comparable<T>> String print(List<Timestamp<T>> res) {
