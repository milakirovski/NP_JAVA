...
class LogSystem <T extends ILog>{
    ...
    public LogSystem(ArrayList<T> elements){
        ...
        //TODO define concrete log processors with lambda expressions
            ...
            ArrayList<T> firstLogProcessorArrayList = new ArrayList<>();
        LogProcessor<T> firstLogProcessor = logsList -> logsList.stream().filter(log -> log.getType().equals("INFO")).collect(Collectors.toCollection(() -> firstLogProcessorArrayList));

        LogProcessor<T> secondLogProcessor = logsList -> logsList.stream().filter(log -> log.getMessage().split("").length < 100).collect(Collectors.toCollection(() -> new ArrayList<>()));

        LogProcessor<T> thirdLogProcessor = logsList -> logsList.stream().sorted(Comparator.comparingLong(ILog::getTimestamp)).collect(Collectors.toCollection(()->new ArrayList<>()));
