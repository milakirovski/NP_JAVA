...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    void printResults() {
        ...
        //TODO 1. Процесор којшто ќе ги врати само логовите коишто се од тип INFO
        LogProcessor<T> firstLogProcessor = (logs) -> logs.stream()
                .filter(i -> i.getType().equals("INFO"))
                .collect(Collectors.toCollection( () -> new ArrayList<>()));

        //TODO 2. Процесор којшто ќе ги врати само логовите чиишто пораки се пократки од 100 карактери
        LogProcessor<T> secondLogProcessor = logs -> logs.stream()
                .filter(i -> i.getMessage().length() < 100)
                .collect(Collectors.toCollection(ArrayList::new));

        // TODO 3. Процесор којшто ќе ги врати логовите сортирани според временскиот печат во растечки редослед (од најстар кон најнов лог).
